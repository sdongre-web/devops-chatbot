...
def ingest_sql_data():
    """Fetch data from MySQL (id, text) and insert into Qdrant."""
    ensure_collection()
 
    db = get_db_connection()
    cursor = db.cursor(dictionary=True)
 
    # Fetch only id and text columns from the pages table for book_id = 10
    cursor.execute("SELECT id, text FROM pages WHERE book_id = %s", (3,))
    ...
    print(f"✅ Successfully ingested {len(rows)} records into Qdrant.")
